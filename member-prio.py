...
parser = argparse.ArgumentParser(description="Script to change a member priority in a service-group")
...
parser.add_argument("--status", default='enable', choices=['enable', 'disable'], help="Set the member port (default: 80)")
...
try:
    ...
    json_post = { 'member': { 'server': service_group_member, 'port': service_group_member_port, 'status': service_group_member_status, 'priority': service_group_member_priority }, 'name': service_group }
    response = axapi_call(session_url + '&method=slb.service_group.member.update', json.dumps(json_post))
    print response
