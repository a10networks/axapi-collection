...
parser = argparse.ArgumentParser(description="Push the Fixed-NAT table \
    from an A10 device")
...
parser.add_argument("--ftp-account", required=True,
                    help="Set the FTP account")
parser.add_argument("--ftp-password", required=True,
                    help="Set the FTP password")
parser.add_argument("--ftp-folder", default='/',
                    help="Set the FTP directory to upload to (default: /)")
parser.add_argument("--table", required=True,
                    help="The Fixed-NAT table to push")
...
try:
    ...
    response = axapi_call(session_url + '&method=cli.deploy',
                          'export fixed-nat ' + fixed_nat_table +
                          ' ftp://' + ftp_upload_account + ':' +
                          ftp_upload_pwd + '@' + ftp_server +
                          ftp_upload_folder + fixed_nat_table)
